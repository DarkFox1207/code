...
// Потокобезопасная оболочка для интерфейса Readable
class SynchronizedReadable implements Readable {
    private final Readable readable;

    public SynchronizedReadable(Readable readable) {
        this.readable = readable;
    }

    @Override
    public synchronized String getName() {
        return readable.getName();
    }

    @Override
    public synchronized int totalContentPages() {
        return readable.totalContentPages();
    }
...
class SerializationUtils {
    ...
    // Метод для создания потокобезопасной оболочки
    public static Readable synchronizedReadable(Readable readable) {
        return new SynchronizedReadable(readable);
    }
}
...
public class Main {
    public static void main(String[] args) {
        try {
            ...
            // Создаем потокобезопасные оболочки
            Readable synchronizedBookSeries = SerializationUtils.synchronizedReadable(bookSeries);
            Readable synchronizedArticleCollection = SerializationUtils.synchronizedReadable(articleCollection);

            System.out.println("До сериализации:");
            synchronizedBookSeries.printInfo();
            synchronizedArticleCollection.printInfo();

            // Сериализация
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            SerializationUtils.serialize(synchronizedBookSeries, baos);
            SerializationUtils.serialize(synchronizedArticleCollection, baos);
