...
class Collections {
    ...
    public static <T> MyCollection<T> createMyCollection() {
        return ((MyCollectionFactory<T>) factory).createInstance();
    }

    @SuppressWarnings("unchecked")
    public static <T> MyCollection<T> createMyCollection(int initialCapacity) {
        return ((MyCollectionFactory<T>) factory).createInstance(initialCapacity);
    }

    @SuppressWarnings("unchecked")
    public static <T> MyCollection<T> createMyCollection(T[] elements) {
...
public class Main {
    public static void main(String[] args) {
        ...
        MyCollection<String> defaultList = Collections.createMyCollection();
        defaultList.add("A");
        defaultList.add("B");
        System.out.println("MyList по умолчанию: " + defaultList.size());

        // Создаем экземпляр MyList с начальной емкостью
        MyCollection<String> listWithCapacity = Collections.createMyCollection(20);
        System.out.println("MyList с начальной емкостью: " + listWithCapacity.size());

        // Создаем экземпляр MyList с начальными элементами
        MyCollection<String> listWithElements = Collections.createMyCollection(new String[]{"X", "Y", "Z"});
        ...
        MyCollection<String> defaultSet = Collections.createMyCollection();
        defaultSet.add("A");
        defaultSet.add("B");
        System.out.println("MySet по умолчанию: " + defaultSet.size());

        // Создаем экземпляр MySet с начальной емкостью
        MyCollection<String> setWithCapacity = Collections.createMyCollection(20);
        System.out.println("MySet с начальной емкостью: " + setWithCapacity.size());

        // Создаем экземпляр MySet с начальными элементами
        MyCollection<String> setWithElements = Collections.createMyCollection(new String[]{"X", "Y", "Z"});
